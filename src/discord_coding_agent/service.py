...
def render(config: Config, executable: Path | None = None) -> str:
...
ExecStart={argument(python, executable=True)} -m discord_coding_agent --config {argument(str(config.path))} run
...
def verify(text: str, name: str = DEFAULT_UNIT) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dca-unit-") as directory:
        path = Path(directory) / unit_name(name)
