...
Current release **0.2.3**, checked **2026-09-10 UTC**: **179 passed, 3 opt-in skips on each** of Python 3.11.16, 3.12.14, 3.13.15 and 3.14.7 on Linux x86-64. Lint/format, strict typing, wheel/sdist build and isolated wheel installation passed. Initialization regression tests exercise the real bridge/RPC timers with schema-validated gated responses: preparation survives the ordinary 45-second cutoff, stages share the 120-second default budget, explicit task deadlines and stop still work, and later turn acknowledgements retain the ordinary timeout. Tests advance the scheduler clock; they do not simulate a live Pi’s underlying startup delay.
