...
Current release **0.2.2**, checked **2026-09-10 UTC**: **172 passed, 3 opt-in skips on each** of Python 3.11.16, 3.12.14, 3.13.15 and 3.14.7 on Linux x86-64. Lint/format, strict typing, wheel/sdist build and isolated wheel installation passed. Delivery tests cover large inline replies, ordinary code fences, Unicode JSON round trips, message limits, priority/fairness, full queues, per-page reconciliation, expired controls and `!last` recovery without model replay. Gateway adapter tests use simulated channel I/O and require no attachment permission. Real mobile-client rendering was not tested here.

The Codex adapter and updater protocol are unchanged. The non-model Codex evidence below was collected on **2026-09-09** and was not rerun for this delivery/documentation patch. A checked configuration setting is not a guarantee of every tool, account or workload’s behavior. No live Discord or Pi service was modified during validation.
