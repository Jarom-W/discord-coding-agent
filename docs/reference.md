...
| `!status full` | Add initialization/RPC budgets, task ID and any interrupted follow-up metadata. |
...
All replies use inline Discord messages, including help, coding results, directory/session listings, approval details and `!last`. Long text is split at line/word boundaries where possible; ordinary fenced code blocks close and reopen across pages. Unicode punctuation is preserved. There is no attachment fallback or eight-page cap; existing result-size and queue bounds still apply. Status and controls can be delivered between result pages. A failed delivery leaves the saved result available through `!last`, which sends a fresh inline copy without invoking Codex.
