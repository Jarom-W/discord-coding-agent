...
`ID` is the request ID displayed by this bridge, not a thread/item/message ID. Read every page of request details before deciding; buttons appear after the complete request. Requests expire after `user_wait` seconds, including time spent delivering details, or when the turn ends, resolves, stops or the service restarts. Reconnects within the same running process retain still-pending requests. Stale controls remain unable to authorize anything even if Discord cannot remove their buttons immediately.

All replies use inline Discord messages, including help, coding results, directory/session listings, approval details and `!last`. Long text is split at line/word boundaries where possible; ordinary fenced code blocks close and reopen across pages. Unicode punctuation is preserved. There is no attachment fallback or eight-page cap; existing result-size and queue bounds still apply. Status and controls can be delivered between result pages. Task failures are saved separately and available through `!status full`, including after restart. The status also reports the last delivery failure observed by the current process. A failed result delivery leaves the saved result available through `!last`, which sends a fresh inline copy without invoking Codex.

Ordinary messages submitted after completion resume the saved conversation. During active work, ordinary text in the same channel is sent to the same turn using `turn/steer`. The bridge confirms acceptance separately from receipt. During startup or another steer RPC, up to eight follow-ups wait in memory (64 KiB per message); they never become independent tasks. Other channels' busy messages are **not submitted**. See [follow-up semantics and recovery](chat.md). The last 512 accepted/handled owner message IDs are deduplicated and persisted, including across process restart. Old deliveries outside that window are not guaranteed deduplicated. Attachments/stickers reject the entire incoming message; paths must be sent as ordinary text. Images, audio and uploaded files are not read.

Asking Codex in ordinary prose to start a new chat does not switch bridge routing. Use `!new`, `!session` or `!repo`. Repository/session/mode/name changes are rejected while any channel is working or maintenance owns the shared lock. See [workspace setup](workspaces.md) for naming, directory roots, additional channels and recovery.
...
| `request` | 45 | One ordinary RPC response, including `turn/start` acknowledgement after preparation and each `turn/steer`. A lost acknowledgement is uncertain work. Never resend automatically. |
...
Thread loading can take longer than a short question suggests: it happens before the question is submitted. It shares the initialization budget; ordinary RPCs retain their separate 45-second default. `!run unlimited` disables only the full-task deadline. `!status` shows the preparation step during initialization; `!status full` retains the last step and timeout budgets; `doctor` prints configured timeout values. Errors name the operation, measured elapsed time, configured limit and diagnostic next step. Human wait counts against its own timeout and any enabled overall deadline. Disabling the task deadline does not disable approval checks, RPC/connection timeouts, `!stop`, Codex completion/failure, or account usage limits. Quiet logs alone never trigger cancellation. HTTP typing attempts have a cosmetic limit of at most 5 seconds and cannot cancel a task.
